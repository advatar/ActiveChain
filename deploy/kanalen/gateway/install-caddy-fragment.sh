...
cleanup() {
  ...
  if (( status != 0 )) && [[ "$installed" == true && -f "$backup" ]]; then
    ...
    "$docker_bin" compose --project-directory "$caddy_dir" \
...
"$docker_bin" compose --project-directory "$caddy_dir" --profile standalone-caddy \
  run --rm --no-deps --entrypoint caddy caddy validate --config /etc/caddy/Caddyfile >/dev/null
"$docker_bin" compose --project-directory "$caddy_dir" --profile standalone-caddy \
  up -d --no-deps --pull never --force-recreate caddy >/dev/null
