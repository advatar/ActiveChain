...
cargo run --quiet -p activechain-consensus-runtime --bin genesis-tool -- \
  "$rehearsal_dir/genesis.bin" 1 1 3 "$rehearsal_dir/validator-keys"
test -s "$rehearsal_dir/genesis.bin"

echo "2/7 provisioning a fresh operator wallet identity"
cargo run --quiet -p activechain-wallet-core --bin activechain-wallet -- \
  derive "$rehearsal_dir/operator-wallet.key" >"$rehearsal_dir/wallet.out"
