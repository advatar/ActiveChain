...
suite registry, exact public-key sizes, purpose compatibility, and temporal bounds. The credential
family independently decodes CredentialStatement, Credential, CredentialStatusRegistry, and
CredentialAcceptancePolicy v1 and recomputes presentation, status-freshness, evidence-binding, and
acceptance outcomes against positive and adversarial language-neutral vectors. These are joined by
complete CapabilityGrant v1 structural decoding and parent/child attenuation checks for
...
cryptographically verify ML-DSA signatures, execute transitions, or replay finalized roots, and is
not M2 evidence. Credential signature suite and exact signature framing are nevertheless enforced;
the semantic credential corpus consumes explicitly preverified issuer/status evidence.
