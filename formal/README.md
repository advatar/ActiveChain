...
| agent authenticator rotation | Kani | `crates/wallet-core/src/agent_authenticator.rs` | accepted rotations strictly advance revision and compromised/deactivated agents cannot rotate | excludes cryptographic, attestation, filesystem, and multi-device refinement; see `formal/KANI_AGENT_AUTHENTICATOR_PROOF_SCOPE.md` |
| bounded consensus traces | Tamarin + Lean/TLA differential evidence | `formal/tamarin/activechain_consensus.spthy`, `formal/lean/ActiveChain/ConsensusHistory.lean`, `formal/tla/ActiveChainConsensus.tla` | authentication, replay, non-equivocation, quorum intersection, frontier lemmas, and a Rust/Lean view-change/restart/epoch history trace | partial; bounded executable refinement exists, but no unbounded production chain-prefix theorem |
...
| finalized-history prefix lifting | Lean 4 | `formal/lean/ActiveChain/ConsensusHistory.lean` | comparable finalized tips imply prefix-comparable full histories; restart preserves the exact history and epoch changes remain parent-bound extensions | comparable tips are an explicit premise; production QC/lock/reconfiguration trace refinement remains open |
...
| native cash and rewards | Lean 4 + differential trace | `formal/lean/ActiveChain/Cash.lean`, `CashAuthorization.lean`, `CashLifecycle.lean` | conservation, authorized issuance, burn, one-shot reward/nullifier use, shielding, restart, and chain-bound spend admission | authoritative Rust ingress and `CashLedger` now have bounded Lean-matching admission/lifecycle evidence; external finality-verifier and unbounded concurrent/block-level refinement remain open |
...
| testnet faucet admission | Kani | `rpc-server::faucet::kani_proofs` | accepted requests satisfy every configured counter/cooldown bound and increased usage cannot convert rejection to acceptance | source provenance, filesystem crash semantics, transaction ingress, finality, and SHAKE256 remain outside this bounded claim; see `KANI_FAUCET_PROOF_SCOPE.md` |
...
| PQ peer sessions | Tamarin | `formal/tamarin/activechain_pq_session.spthy` | 11 symbolic suite/context/replay, exact peer-correspondence, first-message-origin, and honest-session-secrecy lemmas | V2 Rust trace alignment exists; parser refinement, unbounded sequencing, and secure erasure remain outside the proof |
