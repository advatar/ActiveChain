...
use std::{env, io::Read as _, process::ExitCode, time::{SystemTime, UNIX_EPOCH}};
...
fn run() -> Result<VerifyResult, String> {
    ...
    let bundle = trust
        .accepted_bundle()
        .map_err(|_| "operator trust bundle unavailable".to_owned())?;
    ...
    if request.schema != REQUEST_SCHEMA || request.operation != "verify_finalized_external_digest_anchor" {
    ...
    let statement = DigestAnchorStatementV1::new(request.application_domain.as_bytes().to_vec(), digest)
        .map_err(|_| "invalid application domain or digest".to_owned())?;
    let statement_envelope =
        encode_envelope(&statement).map_err(|_| "could not encode expected anchor statement".to_owned())?;
    let reference = statement
        .submission_reference()
        .map_err(|_| "could not derive anchor submission reference".to_owned())?;

    let record_bytes = decode_hex(&request.finalized_record_envelope_hex, "finalizedRecordEnvelopeHex")?;
    ...
    let native_envelope =
        encode_envelope(native).map_err(|_| "could not encode native finality evidence".to_owned())?;
    ...
    let proof_bytes = decode_hex(&request.anchor_state_proof_envelope_hex, "anchorStateProofEnvelopeHex")?;
