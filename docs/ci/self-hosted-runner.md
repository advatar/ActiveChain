...
The runner uses the host's pinned `rustup` and `elan` installations. The workflow installs Rust 1.97.1 and Lean 4.32.0 by exact version, then builds the Rust kernel, executable Lean semantic models, canonical vectors, and shared differential tables entirely on this machine.
