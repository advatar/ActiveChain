...
private enum PaymentRequestFlow {
    ...
    static let journalService = "dev.activechain.payment-request.v1"
    ...
    struct AmountWords: Equatable, Codable {
        ...
        init(atomic: String) throws {
            ...
            guard high != 0 || low != 0 else { throw WalletPaymentFlowError("Amount must be greater than zero.") }
            self.high = high
            self.low = low
        }
    ...
    static func load(wallet: WalletLiveState) throws -> Journal {
        ...
        return Journal(owner: profile.owner, genesis: profile.chainGenesis, nextNonce: 0, pending: nil)
    }
    ...
    static func save(_ journal: Journal, wallet: WalletLiveState) throws {
        ...
        guard bytes.count <= 1_048_576 else { throw WalletPaymentFlowError("Payment journal is too large.") }
        try SharedKeychain().save(bytes, service: journalService, account: wallet.network.id)
    }
    ...
    static func buildApproval(verified: VerifiedWalletPaymentRequest, amount: AmountWords,
                              wallet: WalletLiveState, height: UInt64, nonce: UInt64) throws -> CanonicalCashApproval {
        ...
        guard let input = coins.first(where: { $0.1.owner == owner && amount.isCovered(by: $0.1.amount) }),
              let reserve = coins.first(where: { $0.0.key != input.0.key && $0.1.owner == owner && $0.1.amount.isAtLeast(fee) }) else {
            throw WalletPaymentFlowError("This payment needs a value Coin Cell and a separate fee Coin Cell.")
        }
        guard height <= UInt64.max - validityBlocks else { throw WalletPaymentFlowError("Invalid finalized height.") }
        var bytes = Data(count: 4096), reference = Data(count: 48), length: UInt32 = 0
        let code = DemoMerchant.withPointers([wallet.network.chainID, owner, verified.recipient,
                                              input.0.key, reserve.0.key, verified.cashSessionID]) { p in
            bytes.withUnsafeMutableBytes { out in reference.withUnsafeMutableBytes { id in
                activechain_wallet_build_cash_intent(
                    p[0], p[1], p[2], p[3], p[4], nonce, p[5], height + validityBlocks, 0,
                    amount.high, amount.low, 0, fee, height + validityBlocks,
                    out.bindMemory(to: UInt8.self).baseAddress, 4096, &length,
                    id.bindMemory(to: UInt8.self).baseAddress)
            } }
        }
        guard code == ACTIVECHAIN_WALLET_OK else { throw WalletPaymentFlowError("Could not construct the canonical payment (\(code)).") }
        let approval = try RustCanonicalApproval.review(Data(bytes.prefix(Int(length))))
        guard approval.sessionID == verified.cashSessionID, approval.recipient == verified.recipient,
              approval.amount == amount.approvalWords else {
            throw WalletPaymentFlowError("Canonical payment does not match the signed request.")
        }
        return approval
    }

    static func signedSession(approval: CanonicalCashApproval, wallet: WalletLiveState, height: UInt64) throws -> Data {
        let custody = try custody()
        let key = try custody.publicKey(slotID: wallet.primarySlotID)
        guard let budget = add128(approval.amount, approval.fee) else { throw WalletPaymentFlowError("Payment budget overflow.") }
        let body = approval.chainID + approval.signer + approval.sessionID + integer(height)
            + integer(approval.sessionExpiresAt) + integer(budget.high) + integer(budget.low)
        ...
        let signature = try custody.sign(slotID: wallet.primarySlotID, payload: payload, minimumVersion: 1,
                                         minimumFinalizedHeight: 0, reason: "Authorize this ActiveChain payment")
        var output = Data(count: 4096), length: UInt32 = 0
        let code = DemoMerchant.withPointers([approval.request, key, signature]) { p in output.withUnsafeMutableBytes { out in
            activechain_wallet_encode_cash_session(p[0], UInt32(approval.request.count), p[1], height, p[2],
                                                   out.bindMemory(to: UInt8.self).baseAddress, 4096, &length)
        } }
        guard code == ACTIVECHAIN_WALLET_OK else { throw WalletPaymentFlowError("Could not authorize the payment session (\(code)).") }
        return Data(output.prefix(Int(length)))
    }

    static func add128(_ lhs: Unsigned128Words, _ rhs: Unsigned128Words) -> Unsigned128Words? {
...
final class PaymentRequestPayState: ObservableObject {
    enum Phase: Equatable { case review, preparing, authorizing, submitting, pending, paid(UInt64), failed(String) }
    ...
    var canPay: Bool {
        switch phase { case .review, .failed: return verified.amountAtomicUnits != nil || !openAmountACT.isEmpty; default: return false }
    }
    ...
    func pay(wallet: WalletLiveState) async {
        ...
        do {
            ...
            guard case let .healthy(height) = wallet.networkState else { throw WalletPaymentFlowError("The network is not healthy yet.") }
            let verifiedAgain = try WalletPaymentRequestService.verify(verified.request, network: wallet.network, finalizedHeight: height)
            let atomic = try verifiedAgain.amountAtomicUnits ?? WalletPaymentRequestService.atomicUnits(openAmountACT)
            let amount = try PaymentRequestFlow.AmountWords(atomic: atomic)
            var journal = try PaymentRequestFlow.load(wallet: wallet)
            try await ensureEnrollment(journal: &journal, wallet: wallet, height: height)
            ...
            let approval = try PaymentRequestFlow.buildApproval(verified: verifiedAgain, amount: amount,
                                                                 wallet: wallet, height: currentHeight, nonce: journal.nextNonce)
            phase = .authorizing
            let session = try PaymentRequestFlow.signedSession(approval: approval, wallet: wallet, height: currentHeight)
            ...
            journal.pending = PaymentRequestFlow.Pending(requestReference: verifiedAgain.reference,
                cashReference: approval.intentID, request: approval.request, session: session, transfer: transfer,
                recipient: verifiedAgain.recipient, amount: amount, outputOrigin: origin)
            ...
            guard receipt.reference == approval.intentID else { throw WalletPaymentFlowError("Payment submission reference mismatch.") }
            phase = .pending
            try await resolvePending(journal.pending!, journal: &journal, wallet: wallet)
    ...
    func refresh(wallet: WalletLiveState) async {
        do {
            var journal = try PaymentRequestFlow.load(wallet: wallet)
            guard let pending = journal.pending, pending.requestReference == verified.reference else { return }
            phase = .pending
            try await resolvePending(pending, journal: &journal, wallet: wallet)
        } catch { phase = .failed(error.localizedDescription) }
    }

    private func ensureEnrollment(journal: inout PaymentRequestFlow.Journal, wallet: WalletLiveState,
                                  height: UInt64) async throws {
        if let enrollment = journal.enrollment,
           let enrolled = try enrollment.verifiedHeight(network: wallet.network, owner: journal.owner), height > enrolled { return }
        if journal.enrollment == nil {
            ...
            guard receipt.reference == reference else { throw WalletPaymentFlowError("Wallet setup submission mismatch.") }
        }
        ...
        for _ in 0..<20 {
            ...
            if receipt.state == 1 {
                ...
                let finalized = try evidence.verifiedHeight(reference: enrollment.reference, network: wallet.network)
                guard receipt.height == finalized else { throw WalletPaymentFlowError("Wallet setup finality mismatch.") }
                enrollment.evidence = evidence; journal.enrollment = enrollment
                try PaymentRequestFlow.save(journal, wallet: wallet); return
            }
            if receipt.state == 3 { _ = try await rpc.enrollKey(enrollment.bytes) }
            try await Task.sleep(for: .seconds(2))
        }
        throw WalletPaymentFlowError("Wallet setup is still finalizing. Try Pay again in a moment.")
    }

    private func resolvePending(_ pending: PaymentRequestFlow.Pending, journal: inout PaymentRequestFlow.Journal,
                                wallet: WalletLiveState) async throws {
        for _ in 0..<20 {
            let receipt = try await rpc.resolveCash(pending.cashReference)
            if receipt.state == 3 { _ = try await rpc.submitPayment(session: pending.session, transfer: pending.transfer) }
            if receipt.state == 1 {
                let evidence = try await rpc.cashEvidence(pending.cashReference)
                let height = try evidence.verifiedHeight(reference: pending.cashReference, network: wallet.network)
                guard receipt.height == height else { throw WalletPaymentFlowError("Payment finality mismatch.") }
                guard let output = try await findRecipientOutput(pending) else {
                    throw WalletPaymentFlowError("Payment finalized; waiting for the recipient proof.")
                }
                try DemoPurchase.verifyOutput(output, owner: pending.recipient, reference: pending.outputOrigin,
                                              index: 0, amount: pending.amount.demoAmount, height: height, network: wallet.network)
                guard journal.nextNonce < UInt64.max else { throw WalletPaymentFlowError("Wallet payment nonce exhausted.") }
                journal.nextNonce += 1; journal.pending = nil
                try PaymentRequestFlow.save(journal, wallet: wallet)
                phase = .paid(height); await wallet.refresh(); return
            }
            try await Task.sleep(for: .seconds(2))
        }
        phase = .pending
    ...
    private func findRecipientOutput(_ pending: PaymentRequestFlow.Pending) async throws -> WalletOwnerCoinRecord? {
        var cursor: Data?
        for _ in 0..<64 {
            let page = try await rpc.ownerCoinCells(owner: pending.recipient, after: cursor)
        ...
        throw WalletPaymentFlowError("Recipient proof history exceeds the wallet query limit.")
    }
    ...
    private static func actText(_ atomic: String) -> String {
        ...
        var fraction = String(padded[split...]); while fraction.last == "0" { fraction.removeLast() }
