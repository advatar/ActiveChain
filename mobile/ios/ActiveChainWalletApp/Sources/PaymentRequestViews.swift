...
struct PaymentRequestReviewSheet: View {
            ...
            ScrollView {
                VStack(spacing: 18) {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        if !state.verified.memo.isEmpty { Divider(); Text(state.verified.memo) }
                        Divider()
                    ...
                    Button(action: { Task { await state.pay(wallet: wallet) } }) {
                        Label(buttonTitle, systemImage: "faceid").frame(maxWidth: .infinity)
                    }
                    .buttonStyle(.borderedProminent).controlSize(.large).disabled(!state.canPay)
                    Text("A payment request cannot spend funds. Only this wallet's biometric approval can authorize the canonical cash transfer.")
                        .font(.caption2).foregroundStyle(WalletPalette.muted).multilineTextAlignment(.center)
                }.padding(20)
            }
    ...
    @ViewBuilder private var status: some View {
        switch state.phase {
        case .review: Label("Ready for review", systemImage: "checkmark.circle").foregroundStyle(WalletPalette.mint)
        case .preparing: progress("Preparing wallet…")
        case .authorizing: progress("Waiting for biometric approval…")
        case .submitting: progress("Submitting payment…")
        case .pending: progress("Payment submitted · verifying finality…")
        case let .paid(height):
            VStack(spacing: 8) {
                Label("Paid", systemImage: "checkmark.seal.fill").font(.title3.bold()).foregroundStyle(WalletPalette.mint)
                Text("Finalized and recipient output verified at block \(height).").font(.caption).foregroundStyle(WalletPalette.muted)
            }.cardStyle()
        case let .failed(message):
            VStack(alignment: .leading, spacing: 8) {
                Label("Payment not sent", systemImage: "exclamationmark.triangle.fill").font(.headline).foregroundStyle(.orange)
    ...
    private var buttonTitle: String { if case .failed = state.phase { return "Try again" }; return "Review & Pay" }
}
                ...
                VStack(spacing: 18) {
                    if let request {
                        WalletPaymentRequestQRCode(request: request)
                            .frame(maxWidth: 280, maxHeight: 280).padding()
                            .background(.white, in: RoundedRectangle(cornerRadius: 24))
                        if let link = try? request.deepLink {
                            ShareLink(item: link) {
                                Label("Share payment request", systemImage: "square.and.arrow.up").frame(maxWidth: .infinity)
                            }.buttonStyle(.borderedProminent).controlSize(.large)
                            Text("Anyone can verify this request, but it cannot authorize a payment from their wallet.")
                                .font(.caption).foregroundStyle(WalletPalette.muted).multilineTextAlignment(.center)
                        }
                    } else {
                        VStack(alignment: .leading, spacing: 14) {
                            Text("Amount").font(.headline)
                            TextField("e.g. 10.50", text: $amount).textFieldStyle(.roundedBorder).walletNumberKeyboard()
                            Text("Leave blank for an open-amount request.").font(.caption).foregroundStyle(WalletPalette.muted)
                            Text("Memo").font(.headline)
                            TextField("What is this for?", text: $memo).textFieldStyle(.roundedBorder)
                        }.cardStyle()
                        Button("Create request") {
                            do {
                                let trimmed = amount.trimmingCharacters(in: .whitespacesAndNewlines)
                                request = try WalletPaymentRequestService().create(amountACT: trimmed.isEmpty ? nil : trimmed, memo: memo)
                                error = nil
